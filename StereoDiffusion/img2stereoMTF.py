...
def rescale_depth(depth, min_val, max_val):
    depth_min = depth.min()
    depth_max = depth.max()
    if depth_max - depth_min > torch.finfo(torch.float32).eps:
        out = (max_val-min_val) * (depth - depth_min) / (depth_max - depth_min) + min_val
    else:
        out = torch.zeros(depth.shape, dtype=depth.dtype)
    return out
...
def run_inv_sd(image,args):
  #  depthmodel_path = args.depthmodel_path
    ...
    #print(image_gt, image_gt.shape)
    #print(image_gt.max())
    #image_gt_ = (image_gt / 255.0).transpose(2,0,1)
  #  print(image_gt_,image_gt_.shape)
   # image_gt = (image_gt / 255.0) # trying the normalization this way?
    #print("image gt norm? ", image_gt, image_gt.shape)
    #Image.fromarray(image_gt).save('./image_gt_test.png')
   # Image.fromarray(image_gt_).save('./image_gt_test_tranpose.png')



    print("showing from left to right: the ground truth image, the vq-autoencoder reconstruction, the null-text inverted image")
    #tensor_name = img_name_base.split("_512")[0]
    tensor_path = f'./Marigold/output/tensor/{img_name_base}_512_pred.pt'
    #tensor_path = f'./Marigold/output/tensor_gt/{img_name_base}.pt'

    print(tensor_path)
    depth_pred = torch.load(tensor_path)
   # depth_pred = torch.load("./test.pt")
    disp = _norm_depth(depth_pred)
...
if __name__ == "__main__":
    ...
    rgb_name_base = os.path.splitext(os.path.basename(args.img_path))[0]
    Image.fromarray(image_pair).save(os.path.join('./outputs/Marigold025',f'{rgb_name_base}.png'))

    #Image.fromarray(image_pair).save(os.path.join('./outputs/Marigold',f'{args.img_path.split("/")[-1]}'))
