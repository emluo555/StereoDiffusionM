...
if "__main__" == __name__:
    ...
    print("args batch_size: ",
        batch_size,processing_res,match_input_res, apple_silicon)
    # -------------------- Preparation --------------------
    # Print out config
    logging.info(
    ...
    output_dir_tensor = os.path.join(output_dir, "tensor")
    os.makedirs(output_dir, exist_ok=True)
    os.makedirs(output_dir_color, exist_ok=True)
    os.makedirs(output_dir_tif, exist_ok=True)
    os.makedirs(output_dir_npy, exist_ok=True)
    os.makedirs(output_dir_tensor, exist_ok=True)
    ...
    with torch.no_grad():
        ...
        for rgb_path in tqdm(rgb_filename_list, desc="Estimating depth", leave=True):
            ...
            # input_image = Image.fromarray(img_arr[:,:,3])
            ...
            print('pipe out depth np ', pipe_out.depth_np.dtype, pipe_out.depth_np.shape )

            depth_pred: np.ndarray = pipe_out.depth_np
            print('depth pred ', depth_pred.dtype, depth_pred.shape )
            print('depth pred actual ', depth_pred)

            tensor = torch.tensor(depth_pred).unsqueeze(0)
            print(tensor.shape,tensor.dtype)
            print(tensor)

            depth_colored: Image.Image = pipe_out.depth_colored

            print('image converted ', depth_colored.size,depth_colored.getbands())
            ...
            # save tensors
            tensor_save_path = os.path.join(output_dir_tensor, f"{pred_name_base}.pt")
            if os.path.exists(tensor_save_path):
                logging.warning(f"Existing file: '{tensor_save_path}' will be overwritten")
