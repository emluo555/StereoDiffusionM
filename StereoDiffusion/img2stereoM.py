...
def run_inv_sd(image,args):
    ...
    subprocess.run(["python","./Marigold/run.py","--input_rgb_dir","./images", "--output_dir","./Marigold/output",])

    #depth_pred = runMarigold("./images/")
    #print('marigold depth ', depth_pred.shape, depth_pred)
    #subprocess.run(["python","./Marigold/run.py","--input_rgb_dir","./images", "--output_dir","./Marigold/output",])

           # Read input image
    # rgb_int = batch["rgb_int"].squeeze().numpy().astype(np.uint8)  # [3, H, W]
    # rgb_int = np.moveaxis(rgb_int, 0, -1)  # [H, W, 3]
   # input_image = Image.fromarray(np.expand_dims(image_gt/255,0).transpose(0,3,1,2)/255,device=device,dtype=torch.float32)
   # input_image = torch.tensor(np.expand_dims(image_gt/255,0).transpose(0,3,1,2)/255,device=device,dtype=torch.float32)
