...
def run_inv_sd(image,args,rgb_path):
  ...
  #  print(image_gt_,image_gt_.shape)
   ...
   # Image.fromarray(image_gt_).save('./image_gt_test_tranpose.png')
    ...
    tensor_path = f'./Marigold/output/tensor/{img_name_base}_pred.pt'
    print(tensor_path)
...
if __name__ == "__main__":
    ...
    for rgb_path in tqdm(rgb_filename_list, desc="stereo generation", leave=True):
        image = load_512(rgb_path)
        out_image,image_pair = run_inv_sd(image,args,rgb_path)
        Image.fromarray(image_pair).save(os.path.join('./outputs',f'{rgb_path}'))

    # i think right now looping it is causing problems 
